...
<body>
	<div class="container">
		<h2 style="margin-top:30px;">Patient status</h2>
	...
	<script>
		...
		webSocket.onmessage = function process(message){
			...
			if(jsonData.message != null){
				...
				row.innerHTML="<td>" + details[0] + "</td><td>" + details[1] + "</td><td>" + details[2] + "</td><td><button class=\"btn btn-danger btn-sm\" onclick=\"send('" + details[0] + "','ambulance," + details[3] + "')\">Summon Ambulance</button><button type=\"button\" class=\"btn btn-primary btn-sm\" onclick=\"send('" + details[0] + "','medication')\">Suggest Medication</button></td>";
			}
		}
		function send(user, message){
			if(message == 'medication'){
				...
				document.getElementById("submit_btn").addEventListener("click",function(){
					...
					webSocket.send(user + ',' + message+',' + medicine+',' + description);
					medicine_name.value="";
